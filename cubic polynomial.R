...
# The model is:
# log(lambda_y) = alpha + beta1 * year_c + beta2 * year_c^2 + beta3 * year_c^3 + log(n_months),
# where lambda_y is the expected number of extreme-hot months in year y,
# and year_c is the centered calendar year.
# This polynomial specification allows for a flexible nonlinear temporal trend.
###
# Region: Ireland bbox
###
# Pipeline:
...
#     M1: t + t^2 + t^3 + factor(month)
#     M2: t + t^2 + t^3 + 1st harmonic
#     M3: t + t^2 + t^3 + 2nd harmonic
...
# Fit Quasi-Poisson trend for yearly extreme counts using polynomial time trend
fit_trend <- function(countv, years, exposure) {
  years_c <- years - mean(years)
  glm(countv ~ years_c + I(years_c^2) + I(years_c^3),
      family = quasipoisson(link = "log"),
      offset = log(exposure))
}

# Extract polynomial trend coefficients from Quasi-Poisson model
ext_trend <- function(glm_fit) {
  ...
  tibble(
    ...
    ci_low = coefs[, "Estimate"] - 1.96 * coefs[, "Std. Error"],
    ci_high = coefs[, "Estimate"] + 1.96 * coefs[, "Std. Error"],
    p_value = coefs[, pcol]
  ) %>%
...
## fit model M1~4
# M1: t + t^2 + t^3 + factor(month)
m1 <- lm(temp ~ year_c + year_c2 + year_c3 + factor(month), data = df)

# M2: t + t^2 + t^3 + 1st harmonic
m2 <- lm(temp ~ year_c + year_c2 + year_c3 + sin1 + cos1, data = df_m2)

# M3: t + t^2 + t^3 + 2nd harmonic
m3 <- lm(temp ~ year_c + year_c2 + year_c3 + sin1 + cos1 + sin2 + cos2, data = df_m3)

# M4: M3 + ARMA errors
xreg_m4 <- as.matrix(df_m3 %>% select(year_c, year_c2, year_c3, sin1, cos1, sin2, cos2))
...
comparison <- tibble(
  model = c("M1: t + t^2 + t^3 + factor(month)",
            "M2: t + t^2 + t^3 + 1st harmonic",
            "M3: t + t^2 + t^3 + 2nd harmonic",
...
# after allowing for a cubic polynomial time trend in the mean structure.
print(comparison)
...
trend_one <- function(residual_vec, years_vec, baseline_years) {
  ...
  fit <- glm(extreme_months ~ years_c + I(years_c^2) + I(years_c^3),
             family = quasipoisson(link = "log"),
             offset = log(n_months),
             data = yearly)
  
  ext_trend(fit) %>%
    mutate(q95 = th$q95, .before = 1)
}
...
# Extract polynomial trend metrics
...
# Under the polynomial time-trend specification, the linear, quadratic, and cubic coefficients
# jointly describe a flexible nonlinear time pattern.
# Interpretation should be based on the fitted curve as a whole rather than on a single per-decade multiplier.
print(trend_table)
