...
# Should we be fitting harmonic model each year and look at trends in coefficients
# or should models we fit over entire time period with time accounted for in 
# model ?
...
n_lon_block <- dim(temp_test_3x3)[1]
n_lat_block <- dim(temp_test_3x3)[2]
...
for (i in 1:n_lon_block) {
  for (j in 1:n_lat_block) {
    ...
    # Second-order harmonic regression with interactions
...
summary(models_list[[1]])
plot(t, temp_test_3x3[1,1,])
lines(fitted(models_list[[1]]), col = "red")
...
AIC_order1 <- matrix(NA, n_lon_block, n_lat_block)
AIC_order2 <- matrix(NA, n_lon_block, n_lat_block)
AIC_order3 <- matrix(NA, n_lon_block, n_lat_block)

BIC_order1 <- matrix(NA, n_lon_block, n_lat_block)
BIC_order2 <- matrix(NA, n_lon_block, n_lat_block)
BIC_order3 <- matrix(NA, n_lon_block, n_lat_block)

best_order_AIC <- matrix(NA, n_lon_block, n_lat_block)
best_order_BIC <- matrix(NA, n_lon_block, n_lat_block)
...
## Trends in amp and phase 

## Compare to analysis from raw data
...
################################################################################
############## Harmonic Regression accounting for Correlated Errors ############
################################################################################
